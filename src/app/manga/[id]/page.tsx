...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try { const media = await getMediaDetails("manga", Number(id)); return { title: media.title, description: media.synopsis?.slice(0, 155) }; }
  catch { return { title: "Manga details" }; }
}
...
export default async function MangaDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    media = await getMediaDetails("manga", id);
  } catch (error) {
