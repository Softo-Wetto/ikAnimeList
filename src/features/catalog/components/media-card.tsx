...
export function MediaCard({ media, priority = false }: { media: MediaSummary; priority?: boolean }) {
  return (
    <article className="group min-w-0">
      <Link className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400" href={`/${media.mediaType}/${media.id}`}>
        <div className="relative aspect-[2/3] overflow-hidden rounded-[1.4rem] bg-zinc-900 shadow-xl shadow-black/20">
          <Image
          ...
          <div className="absolute inset-x-3 bottom-3 flex flex-wrap gap-1.5">
            {media.format ? <Badge className="border-white/10 bg-black/55 text-zinc-100">{media.format}</Badge> : null}
            {media.year ? <Badge className="border-white/10 bg-black/55 text-zinc-100">{media.year}</Badge> : null}
          </div>
        </div>
